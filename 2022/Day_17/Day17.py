...
rocks = ((0,1,2,3), (1,0+1j,2+1j,1+2j), (0,1,2,2+1j,2+2j), (0,0+1j,0+2j,0+3j), (0,1,0+1j,1+1j))
jets = [ord(x)-61 for x in open('input.txt').read()]
i, j = 0, 0
tower, cache, top = set(), dict(), 0
empty = lambda position: position.real in range(7) and position.imag>0 and position not in tower
check = lambda position, direction, rock: all(empty(position+direction+r) for r in rock)
